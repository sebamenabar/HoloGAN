...
def calculate_frechet_distance(real_images, fake_images):
    ...
    real_act = real_pred.view(real_pred.size(0), real_pred.size(1)).cpu().numpy()
    fake_act = fake_pred.view(fake_pred.size(0), fake_pred.size(1)).cpu().numpy()
    ...
    return _calculate_frechet_distance(real_mu, real_sigma, fake_mu, fake_sigma)
