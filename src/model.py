...
from tensorflow.keras.layers import Dense, Conv2D, Flatten, LeakyReLU, Conv2DTranspose, Conv3DTranspose
...
class ObjectGenerator(Model):
    def __init__(
        ...
        self.w = tf.Variable(tf.random.normal((*w_shape, w_dim), stddev=0.02), trainable=True)
        
        self.deconvs = []
        self.adains = []
        for filters, ks, stride in zip(upconv_flters, upconv_ks, upconv_strides):
            self.deconvs.append(Conv3DTranspose(
                filters=filters,
                kernel_size=ks,
                strides=stride,
                padding='same',
    ...
    def call(self, z, view_in=None):
        ...
        '''
        z_dim = z.shape[-1]
        assert z_dim == self.z_dim, f'Input dimension ({z_dim}) does not match expected value ({self.z_dim})'
...
class Generator(Model):
    def __init__(
        ...
        for f, k, s in zip(filters, ks, strides):
            self.deconvs.append(Conv2DTranspose(
                filters=f,
                kernel_size=k,
                strides=s,
                padding='same',
    ...
    def call(self, z_bg, z_fg):
        ...
        bg = self.bg_generator(z_bg, None) # (bsz, 1, height, width, depth, num_channels)
        fg = self.fg_generator(z_fg, None) # (bsz, num_objects, height, width, depth, num_channels)
